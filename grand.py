...
class GRANDAttBlock(MessagePassing):
    ...
    def forward(  # noqa: F811
        ...
        return (alpha - torch.eye(alpha.shape[0], device=alpha.device)) @ x
...
class GRANDBlock(nn.Module):
    def __init__(self, node_dim, num_nodes, num_heads):
        super().__init__()
        ...
        self.ode_attention_func = ODEFunc(
            GRANDAttBlock(
    ...
    def forward(self, x, time_to_integrate):
        ...
        return torchdiffeq.odeint(
            self.ode_attention_func,
            x,
            torch.linspace(time_to_integrate),
            method="dopri5",
        )


class DiffusionRegressor(nn.Module):
    def __init__(self, node_dim, num_nodes, num_heads, out_dim):
        super().__init__()
        self.node_dim = node_dim
        self.num_nodes = num_nodes
        self.num_heads = num_heads
        ...
        self.encoder = GRANDBlock(self.node_dim, self.num_nodes, self.num_heads)
        self.decoder = GRANDBlock(self.node_dim, self.num_nodes, self.num_heads)
